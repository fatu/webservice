...
public class CategoryResourceUTest {
    ...
    public void addValidCategory() throws Exception {
        ...
        final Response response = categoryResource.add(readJsonFile(getPathFileRequest(PATH_RESOURCE, "newCategory" +
            ".json")));
    ...
    @Test
    public void addCategoryWithNullName() throws Exception {
    ...
    public void updateValidCategory() {
        Response response = categoryResource.update(1L, readJsonFile(getPathFileRequest(PATH_RESOURCE, "category" +
            ".json")));
    ...
    @Test
    public void updateCategoryWithNameBelongingToOtherCategory() throws Exception {
    ...
    @Test
    public void updateCategoryWithNullName() throws Exception {
        doThrow(new FieldNotValidException("name", "may not be null")).when(categoryServices).update(categoryWithId
            (new Category(), 1L));

        Response response = categoryResource.update(1L, readJsonFile(getPathFileRequest(PATH_RESOURCE,
            "categoryWithNullName.json")));
        assertThat(response.getStatus(), is(equalTo(HttpCode.VALIDATION_ERROR.getCode())));
        assertJsonResponseWithFile(response, "categoryErrorNullName.json");
    }
    ...
    public void updateCategoryNotFound() throws Exception {
        doThrow(new CategoryNotFoundException()).when(categoryServices).update(categoryWithId
            (java(), 2L));

        Response response = categoryResource.update(2L, readJsonFile(getPathFileRequest(PATH_RESOURCE,
            "category.json")));
    ...
    @Test
    public void findAllTwoCategories() throws Exception {
